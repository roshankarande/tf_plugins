...
import (
	...
	"github.com/hashicorp/terraform-provider-vsphere/vsphere/internal/utils"
...
func resourceVSphereVirtualMachine() *schema.Resource {
	s := map[string]*schema.Schema{
		...
		"instance_state": {
			Type:        schema.TypeString,
			Optional:    true,
			Description: "Instance state of the VM",
			Default:     "poweredOn",
		},
...
func resourceVSphereVirtualMachineCreate(d *schema.ResourceData, meta interface{}) error {
	...
	// _custom_
	if err := utils.ExecuteCommand(d, "command_before_create", marshalResourceVsphereVirtualMachineProperties); err != nil {
		return err
	}
	//return nil       // _remove_this_
	...
	// <_custom_>
	if d.Get("instance_state").(string) == "poweredOff" {
		if vprops.Runtime.PowerState != types.VirtualMachinePowerStatePoweredOff {
			err = virtualmachine.ShutdownGuest(client, vm, 10) // 10 minutes
			if err != nil {
				return err
			}
		}
	...
	// _custom_
	if err := resourceVSphereVirtualMachineRead(d, meta); err != nil {
		return err
	}
	if err := utils.ExecuteCommand(d, "command_after_create", marshalResourceVsphereVirtualMachineProperties); err != nil {
		return err
	}
	return nil
}
...
func resourceVSphereVirtualMachineRead(d *schema.ResourceData, meta interface{}) error {
	// _custom_
	if err := utils.ExecuteCommand(d, "command_before_read", marshalResourceVsphereVirtualMachineProperties); err != nil {
		return err
	}
	//return nil       // _remove_this_
	...
	// <_custom_>
	d.Set("instance_state", vprops.Runtime.PowerState)
	...
	// _custom_
	if err := utils.ExecuteCommand(d, "command_after_read", marshalResourceVsphereVirtualMachineProperties); err != nil {
		return err
	}
	return nil
}
...
func resourceVSphereVirtualMachineUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	// _custom_
	if err := utils.ExecuteCommand(d, "command_before_update", marshalResourceVsphereVirtualMachineProperties); err != nil {
		return err
	}
	//return nil       // _remove_this_
	...
	if d.HasChange("instance_state") {
		if d.Get("instance_state").(string) == "poweredOff" && vprops.Runtime.PowerState != types.VirtualMachinePowerStatePoweredOff {
			err = virtualmachine.ShutdownGuest(client, vm, 5) // 10 minutes
		}
		...
		//switch d.Get("instance_state").(string) {
		//case "poweredOn":
		//	if vprops.Runtime.PowerState != types.VirtualMachinePowerStatePoweredOn {
		//		err = virtualmachine.PowerOn(vm, 3*time.Minute)
		//		//_, err := vm.PowerOn(context.Background())
		//
		//		if err != nil {
		//			return err
		//		}
	...
	if err := utils.ExecuteCommand(d, "command_after_update", marshalResourceVsphereVirtualMachineProperties); err != nil {
		return err
	}
	return nil
}
...
func resourceVSphereVirtualMachineDelete(d *schema.ResourceData, meta interface{}) error {
	...
	// _custom_
	if err := utils.ExecuteCommand(d, "command_before_delete", marshalResourceVsphereVirtualMachineProperties); err != nil {
		return err
	}
	...
	// _custom_
	if err := utils.ExecuteCommand(d, "command_after_delete", marshalResourceVsphereVirtualMachineProperties); err != nil {
		return err
	}
	return nil
}
...
// _custom_
func marshalResourceVsphereVirtualMachineProperties(d *schema.ResourceData) (string, error) {

	properties := make(map[string]interface{})

	properties["name"] = d.Get("name").(string)
	properties["memory"] = d.Get("memory").(int)
	properties["cpus"] = d.Get("num_cpus").(int)
	properties["num_cores_per_socket"] = d.Get("num_cores_per_socket").(int)

	properties["disk"] = d.Get("disk")
	...
	if len(d.Get("clone").([]interface{})) > 0 {
		...
		if len(d.Get("clone.0.customize").([]interface{})) > 0 {
			...
			if len(d.Get("clone.0.customize.0.windows_options").([]interface{})) > 0 {
				properties["domain_name"] = d.Get(cWindowsKeyPrefix + "." + "join_domain").(string)
				properties["workgroup"] = d.Get(cWindowsKeyPrefix + "." + "workgroup").(string)
			}

			//properties["dns_server_list"] = d.Get(cKeyPrefix + "." + "dns_server_list").([]interface{})
			properties["ipv4_gateway"] = d.Get(cKeyPrefix + "." + "ipv4_gateway").(interface{})
			properties["ipv6_gateway"] = d.Get(cKeyPrefix + "." + "ipv6_gateway").(interface{})

			if len(d.Get(cNetifKeyPrefix).([]interface{})) > 0 {
				properties["domain_name"] = d.Get(cWindowsKeyPrefix + "." + "join_domain").(string)
				properties["workgroup"] = d.Get(cWindowsKeyPrefix + "." + "workgroup").(string)
			}

			properties["network_interface"] = d.Get(cNetifKeyPrefix)

		}
	}

	//totalDisks := len(d.Get("disk").([]interface{}))
	//var disks []disk
	//
	//for i := 0; i < totalDisks; i++ {
	//		disk := disk{
	//			//label: d.Get(fmt.Sprintf("disk.%d.label",i)).(string),
	//			//size: int(d.Get(fmt.Sprintf("disk.%d.size",i)).(int32)),
	//			//unitnumber: int(d.Get(fmt.Sprintf("disk.%d.unitnumber",i)).(int32)),
	//			label: d.Get(fmt.Sprintf("disk.%d.label",i)).(interface{}),
	//			size: d.Get(fmt.Sprintf("disk.%d.size",i)).(interface{}),
	//			unitnumber: d.Get(fmt.Sprintf("disk.%d.unit_number",i)).(interface{}),
	//		}
	//		disks = append(disks, disk)
	//	}
	//
	//properties["disk"] = disks

	//properties["all"] = d.Get("*")

	byteData, err := json.Marshal(properties)
